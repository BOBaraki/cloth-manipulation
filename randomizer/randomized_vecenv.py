...
from .randomized_dummy_vecenv import RandomizedDummyVecEnv
...
def make_vec_envs(env_name, seed, num_processes):
    if num_processes==1:
        ...
        envs = RandomizedDummyVecEnv(envs)
    else:
        envs = [make_env(env_name, seed, i)
                for i in range(num_processes)]
    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                remote.send((env.observation_space, env.action_space, env.spec, env.unwrapped.randomization_space))
...
class RandomizedSubprocVecEnv(VecEnv):
    ...
    def __init__(self, env_fns):
        ...
        observation_space, action_space, self.spec, randomization_space = self.remotes[0].recv()
